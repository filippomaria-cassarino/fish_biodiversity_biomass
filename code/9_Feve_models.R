## File: 9_Feve_models.R
## Purpose: model the response of functional evenness to warming and fishing
## Author: Filippomaria Cassarino
## Date: 
## ------------------------------------------------------------------------ ----
## Notes ----

# Only the model all model no fishing showed no selection problems. 
...
boxplot(check_data$Feve, horizontal = TRUE) # no majour outliers
plot(density(check_data$Feve)) # use gaussian distribution in modelling
...
# Selection
all_model <- model_selection(directory = "models/Feve/all/no_fishing",
                             name = "all",
                             data = all_data,
                             mesh_cutoff = 65, 
...
model_validation(model = all_model,
                 data = all_data,
                 directory = "models/Feve/all/no_fishing")
...
# Variables distribution and out-liers were assessed in 6_biomass_models.R
boxplot(arctic_data$Feve, horizontal = TRUE) # no majour outliers
plot(density(arctic_data$Feve)) # use gaussian distribution in modelling
...
model_validation(model = arctic_model,
                 data = arctic_data,
                 directory = "models/Feve/arctic/no_fishing")
...
# Variables distribution and out-liers were assessed in 6_biomass_models.R
boxplot(boreal_data$Feve, horizontal = TRUE) # no majour outliers
plot(density(boreal_data$Feve)) # use gaussian distribution in modelling
...
# Selection
boreal_model <- model_selection(directory = "models/Feve/boreal/no_fishing",
                                name = "boreal",
                                data = boreal_data,
                                mesh_cutoff = 65, 
                                family = gaussian(link = "identity"),
...
model_validation(model = boreal_model,
                 data = boreal_data,
                 directory = "models/Feve/boreal/no_fishing")
...
boxplot(check_data$Feve, horizontal = TRUE) # no majour outliers
plot(density(check_data$Feve)) # use gaussina distribution in modelling
...
# Selection
all_model <- model_selection(directory = "models/Feve/all/with_fishing",
                             name = "all",
                             data = all_data,
                             mesh_cutoff = 65, 
                             family = gaussian(link = "identity"),
...
model_validation(model = all_model,
                 data = all_data,
                 directory = "models/Feve/all/with_fishing")
...
# Selection
arctic_model <- model_selection(directory = "models/Feve/arctic/with_fishing",
                                name = "arctic",
                                data = arctic_data,
                                mesh_cutoff = 65, 
                                family = gaussian(link = "identity"),
...
model_validation(model = arctic_model,
                 data = arctic_data,
                 directory = "models/Feve/arctic/with_fishing")
...
# Selection
boreal_model <- model_selection(directory = "models/Feve/boreal/with_fishing",
                                name = "boreal",
                                data = boreal_data,
                                mesh_cutoff = 65, 
                                family = gaussian(link = "identity"),
...
model_validation(model = boreal_model,
                 data = boreal_data,
                 directory = "models/Feve/boreal/with_fishing")
